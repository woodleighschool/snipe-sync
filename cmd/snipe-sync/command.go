...
func newRunCommand(configPaths *[]string) *cobra.Command {
	...
	command := &cobra.Command{
		Use:   "run",
		Short: "Build and apply one reconciliation plan",
		Args:  cobra.NoArgs,
		RunE: func(command *cobra.Command, _ []string) error {
			if !once {
				return fmt.Errorf("run requires --once")
			}
			service, err := loadService(*configPaths)
			if err != nil {
				return err
			}
			result, reconcileErr := service.Reconcile(command.Context(), true)
			if reconcileErr != nil && result.Apply == nil {
				return reconcileErr
			}
			outputErr := writeRunResult(command.OutOrStdout(), result)
			return errors.Join(reconcileErr, outputErr)
		},
	}
...
func loadService(configPaths []string) (*app.Service, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("start snipe-sync: %w", err)
	}
	return service, nil
}
